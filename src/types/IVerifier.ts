...
export class VerifierMaterial implements IVerifierMaterial {
    ...
    if (data) {
      Object.assign(this, {
        ...
        fecha_calibracion: data.fecha_calibracion
          ? dayjs(data.fecha_calibracion, "YYYY-MM-DD").format("DD/MM/YYYY")
          : null,
      });
    }
  }
  ...
  prepare() {
    return {
      ...this,
      marca:
        this.marca && typeof this.marca === "object"
          ? this.marca.value
          : this.marca,
      fecha_calibracion: this.fecha_calibracion
        ? dayjs(this.fecha_calibracion, "DD/MM/YYYY").format("YYYY-MM-DD")
        : null,
      fecha_vencimiento: this.fecha_calibracion
        ? dayjs(this.fecha_calibracion, "DD/MM/YYYY")
            .add(1, "year")
            .format("YYYY-MM-DD")
        : null,
    };
